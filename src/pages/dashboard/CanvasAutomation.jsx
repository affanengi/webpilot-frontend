             ...
             {executions.length === 0 ? (
                 ...
                 executions.map(ex => (
                    <button 
                       ...
                       className={`w-full text-left p-4 rounded-xl border transition-all ${activeExecutionId === ex.id ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30" : "border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-zinc-600 bg-white dark:bg-zinc-800"}`}
                    >
                       <div className="flex justify-between items-center mb-1.5">
                          <span className="text-xs font-bold text-gray-900 dark:text-gray-100">
                            {ex.timestamp ? new Date(ex.timestamp.toDate()).toLocaleString() : "Just now"}
                          </span>
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${(ex.status||"").toLowerCase() === 'completed' || (ex.status||"").toLowerCase() === 'successful' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : (ex.status||"").toLowerCase().includes('failed') || (ex.status||"").toLowerCase().includes('error') ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                              {ex.status}
                          </span>
                       </div>
